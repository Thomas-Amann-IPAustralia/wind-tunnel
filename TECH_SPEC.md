...
`THRESHOLD_DRAFTING` (two generalists) and `FULL_DRAFTING`/`FULL_REVISING` (six specialists) run their agents concurrently within a single Actions job (async fan-out over Gemini calls), respecting the rate-limit budget (§13). Concurrency here is *within* one run and one job, so there is no repo-commit race between the agents — they write distinct files and a single commit closes the state. Both fan-outs — the generalist pair and the specialists — run through one shared runner (`pipeline/stages/fanout.py`) whose width is `specialist_concurrency` in `config/budgets.yml` (§13; named for the binding six-specialist case, bounded below six for rate headroom, raise as quota allows), and the no-commit-race property is held structurally: workers compute and narrate only, while every file write and every commit — including the per-draft progress commits — is made by the single coordinating thread.
