...
```jsonc
{
  ...
  "revisions": {                      // user-driven revision counts, cap 2 each (§7, brief §7)
    "outline": 0, "poc": 0, "flow_map": 0, "threshold": 1, "full": 0
  },
...
| `POST /api/runs/{id}/flow-map` | Generate flow map | Mermaid → SVG at generation time; both committed |
| `POST /api/runs/{id}/revise` | Revise an artefact | Body `{artefact: "outline"\|"poc"\|"flow_map"\|"threshold"\|"full", instructions}` — the single entry point for all post-generation revisions; enforces the ≤2 per-artefact cap from `run.json.revisions`. Brainstorm artefacts regenerate from the amended outline rather than patching (brief §4). `threshold` (valid only while paused at `THRESHOLD_REVIEW`) re-runs `THRESHOLD_RECONCILING` with the instructions in context — the two generalist drafts stand untouched, preserving their independence, and the engine recomputes. `full` (valid only at `COMPLETE`) dispatches `USER_REVISION` (§5.8) |
