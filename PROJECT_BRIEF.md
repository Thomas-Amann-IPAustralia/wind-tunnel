...
Access is a public URL with no authentication in v1 — obscurity plus a usage warning is the agreed posture. The warning must be prominent, worded plainly, and shown before any input is accepted: everything submitted or generated is stored in a public GitHub repository and is world-readable, so users must not enter classified information, sensitive information, or personally identifiable information. The warning discloses public availability and asks for that judgement call — it must not claim any particular sensitivity ceiling or accreditation for the system itself. The Governance phase output should note in the notebook's provenance cell that the submitting officer attested the inputs contained no sensitive, classified, or personal information appropriate to a public, world-readable repository.
